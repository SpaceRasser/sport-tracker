...
type Rule =
  ...
  | { type: 'inactivity_days_gte'; days: number; cooldownDays?: number; priority?: number }
  | { type: 'workouts_in_days_lt'; days: number; threshold: number; cooldownDays?: number; priority?: number }
  | { type: 'no_activity_in_days'; activityCode: string; days: number; cooldownDays?: number; priority?: number }
  | { type: 'activity_present'; activityCode: string; days: number; cooldownDays?: number; priority?: number }
  | { type: 'run_load_increase_pct_gt'; days: number; pct: number; cooldownDays?: number; priority?: number }
  | { type: 'plateau_metric'; activityCode: string; metricKey: string; days: number; cooldownDays?: number; priority?: number }
...
function daysBetween(a: Date, b: Date) {
  return Math.floor((dayStart(b).getTime() - dayStart(a).getTime()) / (24 * 60 * 60 * 1000));
}
...
export class RecommendationsService {
  ...
  async getForUser(userId: string) {
    ...
    const templates = await this.prisma.recommendationTemplate.findMany({
      where: { active: true },
      select: { id: true, code: true, title: true, description: true, rule: true, active: true, createdAt: true },
    });

    // 3) существующие рекомендации (для cooldown/dedupe)
    const existing = await this.prisma.userRecommendation.findMany({
      where: { userId },
      select: { id: true, templateId: true, createdAt: true, dismissedAt: true },
      orderBy: { createdAt: 'desc' },
    });

    const lastByTemplate = new Map<string, Date>();
    for (const r of existing) {
      if (!lastByTemplate.has(r.templateId)) lastByTemplate.set(r.templateId, r.createdAt);
    }
    ...
    for (const t of templates) {
      ...
      scored.push({
        ...
        text,
        priority,
        meta: { vars: check.vars, reason: check.reason, templateCode: t.code },
        cooldownDays,
      });
    }
    ...
    // ✅ пуш “Новые советы” + антиспам (раз в сутки) + настройка enabled
if (createdIds.length > 0) {
  ...
  if (s.enabled && s.recommendationsEnabled) {
    ...
    // 6) отдаём список не-dismissed (с join шаблона)
    const rows = await this.prisma.userRecommendation.findMany({
      where: { userId, dismissedAt: null },
      orderBy: { createdAt: 'desc' },
      include: {
        template: { select: { id: true, code: true, title: true, description: true } },
      },
    });
    ...
    const items = rows.map((r) => ({
      ...
      text: (r.meta as any)?.vars ? renderTemplate(r.template.description, (r.meta as any).vars) : r.template.description,
      reason: (r.meta as any)?.reason ?? null,
    }));
  ...
  async dismiss(userId: string, id: string) {
    ...
    const rec = await this.prisma.userRecommendation.findFirst({ where: { id, userId }, select: { id: true } });
  ...
  private async computeFeatures(userId: string) {
    ...
    const [profile, lastWorkout, totalWorkouts] = await Promise.all([
      this.prisma.profile.findUnique({ where: { userId }, select: { heightCm: true, weightKg: true, level: true } }),
      this.prisma.workout.findFirst({ where: { userId }, orderBy: { startedAt: 'desc' }, select: { startedAt: true } }),
    ...
    const daysSinceLastWorkout = lastWorkout?.startedAt ? daysBetween(lastWorkout.startedAt, now) : 999;

    const workoutsLast7 = await this.prisma.workout.count({
      where: { userId, startedAt: { gte: new Date(now.getTime() - 7 * 86400000) } },
    });
    ...
    const daysSinceLastStrength = lastStrength?.startedAt ? daysBetween(lastStrength.startedAt, now) : 999;
    ...
    for (let i = 0; i < 14; i++) {
      ...
      const c = await this.prisma.workout.count({ where: { userId, startedAt: { gte: d0, lt: d1 } } });
      if (c > 0) streak++;
      else break;
    }
    ...
    const plateau = await this.plateauMetric(userId, 'RUN', 'avg_pace_min_km', 21);
  ...
  private async runLoadIncreasePct(userId: string, days: number) {
    ...
    const pct = prev > 0 ? Math.round(((last - prev) / prev) * 100) : (last > 0 ? 999 : 0);
    return { last: round1(last), prev: round1(prev), pct };
  }

  private async plateauMetric(userId: string, activityCode: string, metricKey: string, days: number) {
    const now = new Date();
  ...
  private async checkRule(userId: string, rule: Rule, f: any): Promise<{ ok: boolean; vars: any; reason?: string }> {
    const vars: any = {
      ...f,
      bmi: f.bmi ?? '—',
    ...
    if (rule.type === 'no_activity_in_days') {
      if (rule.activityCode === 'GYM') {
        return { ok: (f.daysSinceLastStrength ?? 999) >= rule.days, vars, reason: `no GYM ${rule.days}d` };
      }
    ...
    if (rule.type === 'activity_present') {
      ...
      const c = await this.prisma.workout.count({
        where: { userId, startedAt: { gte: from }, activityType: { code: rule.activityCode } },
      });
      return { ok: c > 0, vars, reason: `has ${rule.activityCode} in ${rule.days}d` };
    }

    if (rule.type === 'run_load_increase_pct_gt') {
      const ok = (f.runLoadIncreasePct ?? 0) > rule.pct && (f.runLoadPrev ?? 0) > 0;
